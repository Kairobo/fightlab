...
CFLAGS_STD   := -std=gnu99 -g -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE -D_REENTRANT -D_GNU_SOURCE \
		-Wall -Wno-unused-parameter -Wno-format-zero-length -pthread -fPIC
LDFLAGS_STD  := -lm -lpthread

ROOT_PATH    := $(subst /dynamixel/common.mk,,$(realpath $(lastword $(MAKEFILE_LIST))))
SRC_PATH     := $(ROOT_PATH)/dynamixel
BIN_PATH     := $(ROOT_PATH)/bin
LIB_PATH     := $(ROOT_PATH)/dynamixel/lib
...
LDFLAGS_COMMON := -L$(LIB_PATH) -lcommon $(LDFLAGS_STD) $(LDFLAGS_GLIB) $(LDFLAGS_GTK)

# math
CFLAGS_MATH  := -I$(SRC_PATH) $(CFLAGS_COMMON) $(CFLAGS_GSL)
LDFLAGS_MATH := -L$(LIB_PATH) -lm $(LDFLAGS_COMMON) $(LDFLAGS_GSL)

# lcmtypes
CFLAGS_LCMTYPES  := -I$(SRC_PATH) -I$(SOLNS_PATH)/dynamixel $(CFLAGS_LCM)
